...
**Hujjat holati:** v2.0 ning loyiha realligi (bepul + yakka dasturchi + nol byudjet) bo'yicha qayta ishlangani
**Sana:** 2026-09-04
...
**Muqobil (agar probe ishlamasa):** overlay'ni `VirtualDisplay` ga tushmaydigan alohida `Display` / `SurfaceControl` qatlamiga chiqarish — texnik jihatdan cheklangan, F0 da tekshiriladi.
...
| FR-108 | S | Scroll Shield (4.3-band) | Scroll paytida blur'siz kadr ko'rinmaydi |
...
**Qaror:** v1 da **ONNX Runtime Mobile**. Konversiya ishi F0/F1 ni bloklamasligi kerak. LiteRT'ga o'tish — v2 uchun ochiq (arxitekturada `InferenceEngine` interfeysi bilan ajratiladi).
...
|---|---|---|---|
| **F0 — Texnik prototip** | 4–6 hafta | Capture + overlay + mask state machine + soxta detektor. Maqsad: **C-01…C-04 ni tekshirish** | Statik NSFW-o'rnini bosuvchi rasm ustida 10 s da 0 flicker; qulf ochish oqimi ≤ 1 bosish |
| **F1 — Model integratsiyasi** | 3–4 hafta | ONNX Runtime + NudeNet 320n + MobileNetV2 gate; golden set yig'ish; threshold kalibratsiyasi | Golden set bo'yicha o'lchangan recall/precision hujjatlashtirildi |
...
**Tavsiya:** F0 ni **eng qisqa yo'l bilan** bajar. Agar C-02 (qulf) yoki C-04 (flicker) hal bo'lmasa — mahsulot g'oyasi qayta ko'rib chiqiladi va 6 oylik mehnat tejaladi.
...
|---|---|---|---|---|
| R-01 | Qulf ochilganda qayta rozilik UX'ni o'ldiradi | **Yuqori** | **Kritik** | F0 da real qurilmada o'lchash; bir bosishlik oqim; hal bo'lmasa g'oyani qayta ko'rish |
...
| R-03 | Mask feedback loop (flicker) hal bo'lmaydi | O'rta | Yuqori | ADR-003, F0 prototipi, muqobil SurfaceControl yo'li |
| R-04 | Tayyor model aniqligi yetarli emas | **O'rta-yuqori** | Yuqori | Golden set bilan halol o'lchash. Modelni yaxshilay olmaymiz — faqat threshold va UX bilan qoplaymiz |
...
| ID | Mavzu | Holat |
|---|---|---|
| ADR-001 | Inference runtime: ONNX Runtime Mobile vs LiteRT | Qaror qabul qilindi (8.2) — yozilishi kerak |
| ADR-002 | Litsenziya: AGPL-3.0 va uning oqibatlari | Qaror qabul qilindi (10.1) — yozilishi kerak |
| ADR-003 | Overlay↔capture flicker: probe strategiyasi | **Ochiq — F0 da hal qilinadi** |
| ADR-004 | Faol paket aniqlash: UsageStats vs AccessibilityService | Qaror qabul qilindi (3.5) |
| ADR-005 | Modellar: APK ichida vs OTA | Qaror qabul qilindi (5.5) |
| ADR-006 | Blur render: RenderEffect vs pre-blurred tiling (API 26–30 fallback) | **Ochiq — F0** |
...
1. **Muvaffaqiyat mezoni.** Qaysi raqamda loyiha "bajarildi"? Taklif: *F0 da flicker 0 va qulf oqimi ≤ 1 bosish; v1.0 da golden set recall ≥ 0.90 va batareya ≤ 12%/soat; 100 ta faol foydalanuvchi.* Tasdiqlash kerak.
...
*Hujjat oxiri. v2.2 uchun: F0 prototip natijalaridan keyin 3, 6 va 14-bo'limlar yangilanadi; ADR-003 va ADR-006 yopiladi.*
