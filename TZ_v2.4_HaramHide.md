...
**Hujjat holati:** F0, F1, F2 va F3 natijalari bilan yangilangan
**Sana:** 2026-09-04
**O'lchov manbai:** [F0](docs/F0-NATIJALAR.md) · [F1](docs/F1-NATIJALAR.md) · [F2](docs/F2-NATIJALAR.md) · [F3](docs/F3-NATIJALAR.md)

---

## 0.AAA. V2.3 GA NISBATAN O'ZGARISHLAR (F2 va F3 dan keyin)

| # | v2.3 dagi holat | v2.4 dagi qaror | Manba |
|---|---|---|---|
| C1 | FR-402: GitHub Pages'dan config; FR-403: versiya tekshiruvi | **Ikkalasi ham OLIB TASHLANDI.** Ular `INTERNET` ruxsatini talab qiladi, u esa endi manifestdan butunlay chiqarilgan. Tarmoq qo'shish ilovaning asosiy da'vosini buzadi | 5.5-band |
| C2 | "INTERNET ruxsati e'lon qilinmagan" | **Bu da'vo YOLG'ON bo'lgan.** `onnxruntime-android` uni manifest birlashtirishida olib kirgan. Endi `tools:node="remove"` bilan chiqarilgan va CI tekshiradi | F3 §1 |
| C3 | APK hajmi ≤ 45 MB (mo'ljal) | **Aniqlashtirildi:** universal APK 145 MB chiqadi. ABI ajratish majburiy. arm64-v8a 43 MB, armeabi-v7a 34 MB | F3 §2 |
| C4 | FR-205 cool-down loyihalangan | **Ishlaydi va o'lchandi.** Ikki mantiqiy xato tuzatildi (bo'sh ro'yxat semantikasi, sozlash paytidagi kechikish) | F2 §3 |
| C5 | ADR-007 ochiq edi | **Yopildi:** sinov paytida mask markazi ochiladi (~20 % maydon), butunlay emas | F2 §2 |
| C6 | Golden set — "yig'ilishi kerak" | **Protokol va vosita tayyor:** `docs/GOLDEN-SET.md` va `tools/evaluate.py`. Vosita qurilmaga qarshi tekshirilgan | GOLDEN-SET.md |
| C7 | Real qurilma testi — "kerak" | **Nazorat ro'yxati tayyor:** `docs/QURILMA-TESTI.md` | QURILMA-TESTI.md |
...
### 3.9. C-15 — Bog'liqliklar manifestga ruxsat qo'sha oladi

> **F3 da topildi. Ilovaning asosiy da'vosi bir necha faza davomida yolg'on bo'lgan.**

`onnxruntime-android:1.29.0` o'z manifestida quyidagilarni e'lon qiladi:

```
android.permission.INTERNET
android.permission.ACCESS_NETWORK_STATE
ai.onnxruntime.TelemetryInitializer   (Microsoft 1DS telemetriyasi)
```

Manifest birlashtirilishida ular chiqqan APK'ga kirgan. Bizning manba
manifestimizda ular hech qachon bo'lmagan.

**Talab (FR-111):**
* Taqiqlangan ruxsatlar `tools:node="remove"` bilan chiqariladi.
* CI tekshiruvi **birlashtirilgan** manifestni ko'rishi shart, manbani emas.
  Faqat manbani tekshiradigan versiya yolg'on "o'tdi" beradi.
* Qabul mezoni: `aapt2 dump permissions <apk>` chiqishida tarmoq ruxsati yo'q.
...
| FR-111 | M | Bog'liqliklar olib kirgan tarmoq ruxsatlari chiqariladi (C-15) | `aapt2 dump permissions` da tarmoq ruxsati yo'q; CI majburlaydi |
...
| ~~FR-402~~ | — | ~~GitHub Pages'dan config~~ | **OLIB TASHLANDI** — tarmoq talab qiladi |
| ~~FR-403~~ | — | ~~Versiya tekshiruvi~~ | **OLIB TASHLANDI** — tarmoq talab qiladi |
| FR-404 | C | Model OTA — **v2 ga suriladi** | |

> **v2.4 qarori.** F3 da `INTERNET` ruxsati manifestdan butunlay chiqarildi
> va CI buni majburlaydi. Demak har qanday tarmoq funksiyasi — hatto
> "ixtiyoriy" bo'lsa ham — ilovaning asosiy da'vosini buzadi.
>
> Almashuv aniq: threshold'larni masofadan sozlash va yangilanish xabari
> yo'qoladi. Ularning o'rniga foydalanuvchi ilovani F-Droid yoki Obtainium
> orqali yangilaydi — bu baribir asosiy tarqatish yo'li edi.
>
> Bu qaror qaytarilsa, README, onboarding matni va `verifyPrivacy` ham
> o'zgarishi shart.
...
| APK hajmi (arm64-v8a, modellar bilan) | 30 MB | **43 MB o'lchandi** | ABI ajratish majburiy — universal APK 145 MB (F3 §2) |
...
| ~~**F2 — MVP (Alpha)**~~ | **BAJARILDI** | Sozlamalar, onboarding, cool-down, whitelist, 3 til | ✅ Kundalik foydalanishga yaroqli |
| **F3 — Beta** | qisman bajarildi | Reliz build, maxfiylik tekshiruvi, F-Droid metadata, jurnal | ⚠️ Ochiq beta va ≥ 10 tashqi foydalanuvchi — hali yo'q |
...
**Qolgan ikki bloklovchi endi bajariladigan vazifaga aylandi:**
1. Golden set — protokol va vosita tayyor: `docs/GOLDEN-SET.md`, `tools/evaluate.py`
2. Qurilma testi — nazorat ro'yxati tayyor: `docs/QURILMA-TESTI.md`
...
| ADR-007 | Probe oynasini qisqartirish (qisman ochish) | ✅ **Yopildi** — F2 §2 |
| ADR-008 | Detektor kirish o'lchami: to'rtburchak vs kvadrat | ✅ Qaror qabul qilindi (F1 §2.1) |
| ADR-009 | Tarmoq funksiyalaridan voz kechish (FR-402/403) | ✅ Qaror qabul qilindi (5.5-band) |
...
*Hujjat oxiri. v2.5 uchun: golden set bo'yicha o'lchovdan keyin 6.3 (aniqlik
metrikalari) va 8.3 (threshold jadvali) haqiqiy qiymatlar bilan almashtiriladi
(`docs/GOLDEN-SET.md` §6); real qurilmada o'lchovdan keyin 3 (C-02, C-03),
6.2 va 14 (R-01) yangilanadi (`docs/QURILMA-TESTI.md` §9).*
